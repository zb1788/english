...
class HxpdataController extends Controller {
...
//处理版本图片
    ...
    public function update_unit_code(){
        $result = M('unit')->field("id,name,ks_code")->where("isdel = 1 and ks_code = '0'")->order("id desc")->select();
          //echo count($result);
          foreach($result as $key=>$value){
             // $sql = "select KS_CODE from share_knowledge_structure where KS_NAME = '".$value['name']."'";
              $rs = M("knowledge_structure","share_")->field("KS_CODE")->where('KS_NAME = "'.$value["name"].'"')->find();
              if(!empty($rs["ks_code"])){
                $id = $value["id"];
                M('unit')->where("id = ".$id)->setField("ks_code",$rs["ks_code"]);
                echo "更新id".$value['id']."\n";
              }
              else{
                echo "error\n".$rs["ks_code"];

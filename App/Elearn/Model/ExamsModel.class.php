...
{
    public function getExamsListData($ks_code){
    	$examsql="select *,case when accnum is null then 0 else accnum end as accnum,case when num is null then 0 else num end as num from engs_exams left join (select paper_id,sum(case when ismark=1 then 1 else 0 end) as accnum,count(*) as num from engs_user_quiz_list where ks_code='%s' group by paper_id) s on engs_exams.id=s.paper_id where ks_code='%s' and isdel=1 and tts_type=1 and state=4";
    ...
    public function getQuestionTTSByExamsid($examsid){
        $sql="select * from engs_exams_questions t where examsid=%d and isdel=1";
        ...
        foreach($rs as $key=>$value){
            ...
            foreach($tts as $tk=>$tv){
                ...
                $temp["url"]=C("exams_mp3_path").substr($tv["tts_mp3"], 0,2)."/".$tv["tts_mp3"].".mp3";
                array_push($arr, $temp);
            }     
        }
        return $arr;
    ...
    public function getQuestionByExamsid($username,$classid,$quizid,$examsid,$index){
        ...
        //这里查询需要优化
        $questionratesql="SELECT answerid,answer,SUM(CASE WHEN iscorrect = 1 THEN 1 ELSE 0  END) AS accnum,COUNT(DISTINCT username) AS answernum  FROM engs_user_quiz_history where issubmit=1 and quizid=".$quizid." and ";
        ...
        //学生作答的汇总数据
        $summarysql="select t.answer,count(*) as per from engs_user_quiz_history t where t.issubmit=1 and t.classid='".$classid."' and t.quizid=".$quizid." and t.questionsid=".$questionid." group by t.answer";

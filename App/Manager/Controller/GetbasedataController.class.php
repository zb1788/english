...
class GetbasedataController extends Controller {
    ...
    public  function getgrade(){
		...
		//if (empty($Data)) {
			...
			$Data = $Dao->field('detail_code,detail_name')->where('dictionary_code like "grade%" and detail_code !="0010"')->order('detail_order')->select();
		//} 
    ...
    public function getversion(){
        ...
        $Dao = M(''); // 实例化Word对象
        if($gradeid=='k0000'){
            $Data = M('rms_dictionary')->field('detail_code,detail_name')->where('dictionary_code ="edition1"')->order('detail_order')->select();
        }else{
            $sql = "SELECT detail_code,detail_name FROM engs_rms_dictionary a WHERE a.detail_code IN (SELECT DISTINCT t.r_version FROM engs_rms_unit t WHERE t.r_grade ='".$gradeid."' AND t.is_unit = 0) AND a.dictionary_code like 'edition%' ORDER BY a.detail_order ASC";
            $Data = $Dao->query($sql);
        }	 

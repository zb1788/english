...
class XygameController extends CheckController {
	...
	public function index(){
		...
		// $username = "410182100000453447";
		$backUrl = I("backUrl/s",'0');
		$mathBackUrl = '';
		if($backUrl !== '0'){
			cookie("mathBackUrl",$backUrl);
			$mathBackUrl = $backUrl;
		}
		else{
			$mathBackUrl = 	cookie("mathBackUrl");
		}
		cookie("moduleid",$moduleid);
		$sql= "select * from (select a.grade,a.term,a.subject,a.name,a.total as stagenum,b.* from db_math.sx_genre a left join db_math.sx_stage b on db_math.a.id = b.genreid where a.subject='".$subjectid."' and grade = '".$gradeid."' and a.isdel = 0 and a.isshow = 1 and b.isdel = 0 order by a.term ,a.sortid,b.sortid) c left join (select stageid,username from db_math.sx_xy_user_stage where username = '".$username."' and status = 1) d on c.id = d.stageid order by c.term,c.sortid";
		// echo $sql;exit;
		$res = M()->query($sql);
	
		$res = array_to_tree($res,'genreid','stagename','name');
		// var_dump($res);
		// redirect("/Subject/Public/treeunit#/mathlist/?gradeid=".$gradeid."&subjectid=".$subjectid."&moduleid=".$moduleid);

		$this->assign("infolist",$res);
		$this->assign("mathBackUrl",$mathBackUrl);
		$this->display();
	}
	...
	public function addUserStage(){
		...
		$res = M('xy_user_stage')->field("id")->where("username='".$username."' and gradeid='".$gradeid."' and subjectid = '0002' and genreid =  ".$genreid." and stageid=".$stageid)->find();
		if(!empty($res)){
			$userstageid = $res["id"];
		}
		else{
	...
	public function getResult(){
		...
		$user_stage = M('xy_user_stage');
		$res = $user_stage->field("addtime")->where('id="%d"',$userstageid)->find();
		...
		if($status == 1){
			...
			if($usertype == 4){
				$ub_sum = $user_stage->where('stageid="%d" AND username="%s" AND localareacode="%s" AND usertype="%d" and status = 1',$stageid,$username,$localareacode,$usertype)->select();
				if(count($ub_sum <=0)){
					$data_stage['status'] = 1;
					$data_stage['score'] = 1;
				}
	...
	$user_stage->where('id="%d"',$userstageid)->save($data_stage);

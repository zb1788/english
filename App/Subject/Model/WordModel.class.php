...
class WordModel extends Model
{
    ...
    private function getWord($wordid){
        $sql='select engs_word.id,engs_word.base_wordid,engs_base_word.word,engs_base_word.usmp3 as ukmp3,engs_base_word.isword,engs_base_word_explains.explains,';
    ...
    public function getWordListData($ks_code,$username){
    	...
    	$sql="select engs_word.isword,engs_word.word,engs_word.id,engs_base_word.ukmark,engs_base_word.usmp3 as ukmp3,engs_base_word.usmp3 as name,";
        $sql=$sql."engs_base_word_explains.morphology,engs_base_word_explains.explains,'1' as size,'mp3' as format,";
        $sql=$sql."concat('".$word_mp3_path."',engs_base_word.usmp3) as url,";
    ...
    public function listenChooseWord($wordid,$username,$areaid){
        //查询这个单词的信息
        ...
        shuffle($items);
        $ret["tncontent"]=PROTOCOL."://".C("word_path").$result[0]["ukmp3"];
        $ret["mp3"]=PROTOCOL."://".C("word_path").$result[0]["ukmp3"];
        $ret["mp3"] = getHttpUrl($ret["mp3"]);
        $ret["wordid"]=$wordid;
    ...
    private function wordChoosePicture($wordid,$username,$areaid){
        ...
        foreach($items as $key=>$value){
            ...
            //获取远程图片
            $items[$key]["path"]=PROTOCOL."://".C("word_pic_path").$value["pic"];
            $filepath=PROTOCOL."://".C("word_pic_path").$value["pic"];
            // $image_info = getimagesize($filepath);
            // $base64_image_content = "data:{$image_info['mime']};base64," . chunk_split(base64_encode(file_get_contents($filepath)));
            $items[$key]["content"]=$filepath;
    ...
    //英译汉
    public function wordTranslateExplain($wordid,$username,$areaid){
        //查询这个单词的信息
        $result=$this->getWord($wordid);
        //构造选项内容
        $items=array();
    ...
    private function wordSpell($wordid,$username,$areaid){
        ...
        $ret["word"]=$result[0]["word"];
        $ret["items"]=$items;
        $ret["isword"]=$result[0]["isword"];
        $ret["ukmark"]=$result[0]["ukmark"];
        $ret["ukmp3"]=$result[0]["ukmp3"];
        $ret["morphology"]=$result[0]["morphology"];
        $ret["explains"]=$result[0]["explains"];
        $ret["pic"]=$result[0]["pic"];
        $answer="";
        foreach($items as $key=>$value){
            $items[$key]["flag"]=chr(65+$key);
            if($result[0]["word"]===$value["word"]){
    ...
    //单词拼写
    public function wordSingleSpell($wordid,$username,$areaid){
        //查询这个单词的信息
        $result=$this->getWord($wordid);
        //构造选项内容
        $itemrs=M("base_word")->where("id=%d",$result[0]["base_wordid"])->find();
        $letters=$itemrs["letters"];
        $others=$itemrs["others"];
        ...
        foreach($itemarr as $key=>$value){
            ...
            $items[$key]["index"]=(array_search($value,$letterarr)===false?-1:array_search($value,$letterarr));
        }
        $ret["items"]=$items;
        $ret["answer"]=$answer;
        $ret["useranswer"]="";
    ...
    private function getChooseList($wordid,$itemsnum){
        ...
        $sql.=' FROM engs_base_word_explains bwe ,(select * from engs_base_word limit '.$ran.',50) AS t1 WHERE  t1.id=bwe.base_wordid  and t1.isword=1  And t1.word!="%s"  AND t1.id!=%d and ';
        $sql.='(t1.word is not null and t1.word!="" and bwe.explains is not null and bwe.explains !="") GROUP BY  t1.word,t1.isword,t1.ukmark,t1.usmark';
        $sql.=' order by rand() LIMIT '.$itemsnum;
        $choicelist = $this->query($sql,$words,$id);
        $ran=mt_rand(0,8000);
        if(count($choicelist)<$itemsnum){
            unset($sql);
            $sql='SELECT t1.word,t1.isword,t1.ukmark,t1.ukmp3,MAX(bwe.morphology) as morphology,MAX(bwe.explains) as explains,MAX(bwe.pic) as pic';
            $sql.=' FROM engs_base_word_explains bwe,(select * from engs_base_word limit '.$ran.',50) AS t1 WHERE  t1.id=bwe.base_wordid and t1.isword=1 AND t1.word not like "%'.$subword.'"';
    ...
    public function getReadWordList($ks_code,$username,$readid,$chapterid){
        ...
        $sql=$sql." where engs_word.ks_code='%s' and engs_word.isdel=1 and engs_base_word.isword=1";

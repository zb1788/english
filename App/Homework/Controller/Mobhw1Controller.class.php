...
class Mobhw1Controller extends CheckController {
  //首页
  	public function examsquiz(){
		...
		$spendtime=$studentlist["papertime"];
		//进行班级的数据的获取
		$classId=I("classId","0");
		//这里需要进行学生班级的查询
    ...
    public function getHomeworkQuestion(){
		...
		if($type==0){
			...
			if($wscount>0&&$index<$wscount){
				...
				$wtsql=$wtsql." order by t.sortid,t.id  limit ".$index.",1";
			...
			$quessql=$quessql." order by t.paper_sortid,t.id limit ".$index.",1";
			...
			$wasql=$wasql." order by t.id limit ".$index.",1";
			//查询结果
			$rss=M()->query($wasql);
			$rs["question"]=$rss[0];
			...
			$tasql=$tasql." AND eh.`chapterid`=t.`chapterid` ORDER BY t.textsortid,t.id limit ".$index.",1";
			$rss=M()->query($tasql);
			$rs["question"]=$rss[0];
			//查询结果
			//查询班级数据班级答题人数以及班级的作答人数 正确率 最高分

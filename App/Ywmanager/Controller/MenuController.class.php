...
class MenuController extends Controller {
    ...
    public function fenye(){
        ...
        $sql_where='where m.r_grade="'.$grade.'" and m.r_subject="'.$subject.'" ';

        $sql1='SELECT l.id,l.title,l.remark,t.sortid,(CASE WHEN t.menuid is not NULL THEN 1 ELSE 0 END) as status,t.isrecommend,t.id as bid FROM engs_menu_modules l LEFT JOIN (SELECT m.menuid,m.sortid,m.isrecommend,m.id FROM engs_menu_config m '.$sql_where.') t ON l.id = t.menuid ';
    ...
    public function addModelToDb(){
        ...
        if($type == 'add'){
            //添加
            $sql = 'INSERT INTO `db_config`.`engs_menu_modules` ( `title`, `url`, `remark`, `style`, `typeid`, `sortid`, `type`) VALUES ( "'.$modelname.'", "'.$modelurl.'", "'.$modelremark.'", "'.$modelstyle.'", "0", NULL, "'.$modeltype.'");';
            $Model->execute($sql);
        }else{
            //修改
    ...
    public function addConfig(){
        ...
        $m = M('db_config.menu_config','engs_');

        $m->where('r_grade="%s" and r_subject="%s"',$grade,$subject)->delete();
        foreach($Arr as $v){
            $v['r_grade'] = $grade;
            $v['r_subject'] = $subject;
            $m->add($v);
    ...
    public function dodata(){
            ...
            $subject=array("0001","0003","0002","0004","0005","0010","0011","0012","0013","0009","0006");
            $subjects=array("语文","英语","数学","物理","化学","生物","地理","政治","历史","品德","音乐");
            $styles=array("bGreen radius8","bYellow radius8","bZi radius8","bGreen radius8","bYellow radius8","bZi radius8","bGreen radius8","bYellow radius8","bZi radius8","bGreen radius8","bYellow radius");
            $arr=array();
            for($i=0;$i<10;$i++){
                $temp=array();

...
function xuanzeHtml(quevalue,stem_type){
    ...
    $(itemsdata).each(function(itemskey,itemsvalue){
    	if(quevalue.itemtype == '0'){	//选项是文字
    		...
    		itemshtml += '<dd><img src="'+resource_path+'/uploads/'+itemsvalue.content+'"><span><input stem_type="'+stem_type+'" name="'+quevalue.id+'" type="radio" value="'+itemsvalue.flag+'">'+itemsvalue.flag+'</dd>';
    	}
    });
...
function show_paper_info(examsid,showtype) {
        ...
        $('#exams_info').show();
       openloading();
    $.getJSON(ListenConter + 'listenshow', {examsid: examsid,unitid:def_unitid,from_platform:from_platform}, function(result) {
        hideloading();
    	htmlstr = '';
    	examsdata = result;

  ...
  createNew: function(data){
      ...
      Exams.initQuestion=function(question){
        ...
        if(question.typeid==1){
          ...
          questionhtml=questionhtml+"><label class='question_types_label'>选择题<font class='tihao'>"+question.question_num+"</font>";
          ...
          questionhtml=questionhtml+"><label class='question_types_label'>填空题<font class='tihao'>"+question.question_num+"</font>";
        }else if(question.typeid==3){
          questionhtml=questionhtml+"<div class='question_item' data-field-type='qunstion_judge' question_num_id='"+question.question_num_id+"' itemtype='"+question.itemtype+"' id='"+question.id+"' question_note='"+question.tips+"' question_stoptime='"+question.stoptimes+"' question_note='"+question.tips+"'  question_num='"+question.tts_content+"' question_num_voice='"+question.voiceid+"' question_stoptime='"+question.stoptimes+"' stemtype='0' tips='"+question.tips+"'";
          $.each(question.keys,function(ktk,ktv){
            questionhtml=questionhtml+" "+ktv.dictionary_code+"='"+ktv.dictionary_id+"' ";
          });
          questionhtml=questionhtml+"><label class='question_types_label'>判断题<font class='tihao'>"+question.question_num+"</font>";
          ...
          questionhtml=questionhtml+"><label class='question_types_label'>排序题<font class='tihao'>"+question.question_num+"</font>";
        }

        questionhtml=questionhtml+"<div class='controlbar clearfix'><a ctrltype='edit' parentid='"+question.id+"' class='edit' title='编辑'></a><a ctrltype='copy' class='copy' title='复制'></a><a ctrltype='delete' id='"+question.id+"' class='delete' title='删除'></a><a ctrltype='collapse' class='collapse_n' title='折叠'></a></div></label>";
        ...
        if(question.typeid==1){
          ...
          $.each(question.items,function(k,v){
            if(question.itemtype==0){
              ...
              questionhtml=questionhtml+"<p><strong id='"+v.id+"'>"+v.flag+"</strong>.<img src='../../uploads/"+v.content+"' width='' height='100px'/></p>";
            }
      ...
      Exams.initPaper = function(){
        ...
        if(exams.tts_type==1){
          ...
          //alert(exams.tts.length);
          if(exams.tts.length>1){
             //播放完成音频展示
          ...
          if(exams.tts!=''){
          $("#ttsupexamsbegin").val(exams.tts[0].tts_content);
          $("#ttsupexamsbegin").parent().find("img").attr("id",exams.tts[0].id);
          $("#ttsupexamsbegin").parent().find("img").attr("mp3",exams.tts[0].ftp_mp3);
          //alert(exams.tts.length);
          if(exams.tts.length>1){
             //播放完成音频展示
            $("#ttsupexamsend").val(exams.tts[1].tts_content);
            $("#ttsupexamsend").parent().find("img").attr("id",exams.tts[1].id);
            $("#ttsupexamsend").parent().find("img").attr("mp3",exams.tts[1].ftp_mp3);
          }
         }
        ...
        $.each(this.papers,function(pk,pv){
          ...
          if(pv.son.length>0){
            ...
            examshtml=examshtml+paperhtml+"</div>";  
          });
        return examshtml;  
      };

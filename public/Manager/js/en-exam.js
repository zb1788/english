...
function questionToController(type){
    ...
    switch (type) {
        case "1":
			...
			console.log(questions.question_note);
			if(questions.question_note==undefined||questions.question_note==''){
				$(li).eq(3).find("select:eq(0)").val("0");
			}
            $(li).eq(3).find("select:eq(0)").val(questions.question_stoptime);
			if(questions.question_stoptime==undefined||questions.question_stoptime==''){
...
function splay(obj){
    var mp3="";
      ...
      if(issys==1){
          ...
          AudioPerform("http://58.22.107.13/voicetemp/"+mp3);
    }else{
          mp3=$(obj).attr("mp3");
...
function savepaper(){
    ...
    $.post("savePaper",{data:encodeURI(JSON.stringify(ex)),ran:Math.random()},function(d){
	 window.location.href = "/Manager/exams/index?ran="+Math.random()+"&examsid="+d.examsid;

...
$.EBC = {
    ...
    setChapter:function(obj,unitid){
        $(obj).empty();
        $.get('../Getbasedata/getChapter', {unitid:unitid,random: Math.random()}, function(data) {
            //alert("aaaaa");
            $.each(data, function(i, value) {
...
function word_check() {   //单词验证，输出验证结果到页面table
    ...
    if (wordlist != "") {
        ...
        $.get('../word/checkdata', {wordlist: wordlist, random: Math.random()}, function(data) { //返回验证结果json
            var i = 0;
            $.each(data, function(i, val) {
                i++;
...
function getWordList()
{
    ...
    if (unitid != '') {
        ...
        $("#table_data td").parents("tr").remove();
        $.getJSON("../word/getlist", {unitid: unitid, random: Math.random()}, function(data) {
            var i = 0;
            $.each(data, function(i, val) {
                i++;
                var tr = $("#table_demo tr").eq(0).clone();
...
function addword() {
    ...
    art.dialog.prompt('请输入要添加的单词', function(val) {
        ...
        var unitid = $("#unitid").val();
        if (val != "") {
            $.get('../word/add', {flag: 1, unitid: unitid, word: val, random: Math.random()}, function(data) {
...
function addphrase() {
    ...
    art.dialog.prompt('请输入要添加的短语', function(val) {
        ...
        var unitid = $("#unitid").val();
        if (val != "") {
            $.get('../word/add', {flag: 0, unitid: unitid, word: val, random: Math.random()}, function(data) {
                art.dialog.tips(data.msg);
                if (data.isadd == "1")
                {
                    getWordList();
...
function upWordsort() {
    ...
    $.get("../word/listup", {data: JSON.stringify(arrjson),ks_code:ks_code}, function(result) {
        getWordList();
        dloading.close();
    });
}
...
function getChapterList(type)
{
    ...
    if (unitid != '') {
        var dloading = art.dialog({time: 30, title: '加载中……', width: 130, height: 30, opacity: 0.3, lock: true});
        $("#table_data td").parents("tr").remove();
        $.getJSON("../text/getchapterlist", {unitid: unitid, random: Math.random()}, function(data) { //获取章节目录json
            var i = 0;
           // alert(data.length);
           if(data.length > 0){
            $.each(data, function(i, val) {
...
function editChapter(id, chapter)
{
    art.dialog.prompt('小节名称', function(val) {
        ...
        var unitid = $("#unitid").val();
        if (val != "") {
            $.get('../text/chapterEdit', {unitid: unitid, id: id, chapter: val, random: Math.random()}, function(data) {
                art.dialog.tips(data.msg);
                if (data.isadd == 1) {
...
function getTextList()
{
    ...
    if (chapterid > 0) {
        var dloading = art.dialog({time: 30, title: '加载中……', width: 130, height: 30, opacity: 0.3, lock: true});
        $("#table_data td").parents("tr").remove();
        $.getJSON("../text/gettextlist", {chapterid: chapterid, random: Math.random()}, function(data) {
            var i = 0;
            $.each(data, function(i, val) {
                i++;
                var tr = $("#table_demo tr").eq(0).clone();
...
function getUnitList() {
    ...
    var termid = $("#termid").val();
        //var dloading = art.dialog({time: 30, title: '加载中……', width: 130, height: 30, opacity: 0.3, lock: true});
        $("#table_data td").parents("tr").remove();
        $.getJSON("../unit/getunitlist", {gradeid: gradeid, versionid: versionid, termid: termid, random: Math.random()}, function(data) {
		//alert(data.length);
            $.each(data, function(i, val) {
 ...
 * @returns {undefined}
 */
function editUnit(ks_code, ks_name_sort) {
    art.dialog.prompt('小节简称', function(val) {
        val = $.trim(val);
        if (val != "") {
            $.get('../unit/unitedit', {ks_code: ks_code, ks_name_short: val, random: Math.random()}, function(data) {
                art.dialog.tips(data.msg);
                getUnitList();
            }, "json");
        }
...
function getversionimg(){
    ...
    var termid = $("#termid").val();
	 var dloading = art.dialog({time: 30, title: '加载中……', width: 130, height: 30, opacity: 0.3, lock: true});
    $("#table_data td").parents("tr").remove();
    $.getJSON("../version/getVersionimg", {r_grade:gradeid,r_volume:termid,random: Math.random()}, function(data) {
        ...
        $.each(data, function(i, val) {
            ...
            td.html(val.detail_name);
			td = tr.children('td').eq(2);
            td.html('<img src="/'+val.pic_path+'" width="180" height="255"/>');
        ...
        callback: function () {  
          ...
          var closeflag = true;
			//alert(r_grade+"=="+r_volume+"=="+r_version+"=="+imgid+"=="+pic);
			  $.get("../version/version_img_update_action?rt=" + Math.random(),{imgid:imgid,r_grade:r_grade,r_volume:r_volume,r_version:r_version,pic:pic},function(data) {

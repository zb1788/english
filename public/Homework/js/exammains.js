        ...
        items.append(
            $("<li></li>").addClass("edi-cell mui-media item radio").attr("type",data.typeid).attr("wordid",data.wordid).attr("questionid",data.quesid).attr("chapterid",data.chapterid).attr("flag",value.flag).append(
                $("<div></div>").addClass("option").append(
                    $('<span></span>').addClass("option_circle").addClass(((answer == value.flag)?"active":"")).attr("quesid",data.quesid).attr("type",data.typeid).html(value.flag)
                )
            ).append(
...
var renderSpell = function(data){
    ...
    var items = $("<ul></ul>").addClass("mui-table-view items");
    $.each(data.items,function(key,value){
        if(data.useranswer != undefined  && data.useranswer.useranswer != null && data.useranswer.useranswer != ""){
            var flag = data.useranswer.useranswer.indexOf(value.content);
            if(flag>=0){
                if(value.content != ""){
                    items.append(
                        $("<li></li>").addClass("edi-cell mui-media item spell checkbox").append(
                            $("<div></div>").addClass("option").append(
                                $('<span></span>').addClass("option_circle").addClass("active").html(value.content)
                            )
                        )
                    );
                }else{
        ...
        if(data.itemtype == 0 || data.typeid == 3){
            items.append(
                $("<li></li>").addClass("edi-cell mui-media item radio").attr("type",data.typeid).attr("wordid",data.wordid).attr("questionid",data.quesid).attr("chapterid",data.chapterid).attr("flag",value.value).append(
                    $("<div></div>").addClass("option").append(
                        $('<span></span>').addClass("option_circle").addClass((data.typeid == 3?((answer == value.value)?"active":""):((answer == value.flag)?"active":""))).attr("quesid",data.quesid).attr("type",data.typeid).html(value.flag)
                    )
                ).append(
                    $("<span></span>").addClass("content").html(value.content)
                )
...
//提交作业事件
document.getElementById("submithomework").addEventListener('tap',function(e){
    issubmit = 1;
    mask.show("正在提交中...");
...
$("body").on("click", ".item.radio", function() {
    ...
    $(this).find(".option_circle").addClass("active");
    var flag = $(this).attr("flag");
    //判断试题是否正确
    var enhomework = window.localStorage.getItem("enhomework");
    var homework_questions = JSON.parse(enhomework);
    //console.log(homework_questions);
    var question = homework_questions.questions[homework_question_index];
    //获取问题属性的所有属性
    var useranswer = {};
    useranswer.useranswer = flag;
    
    if(question.datatype == 2){
        useranswer.userscore = (flag == question.answer)?question.score:0;
        useranswer.iscorrect = (flag == question.answer)?1:0;
    }else{
        useranswer.userscore = (flag == question.answer.answer)?question.score:0;
        useranswer.iscorrect = (flag == question.answer.answer)?1:0;
    }
    useranswer.spendtime = (new Date().getTime() - question_start_time)/1000;
    homework_questions.questions[homework_question_index].useranswer = useranswer;
    window.localStorage.setItem("enhomework",JSON.stringify(homework_questions));
    var user = {};
    user.studentid = studentid;
    user.classid = classid;
    $.post("../Mobhw/userAnswerQuestion",
        {
...
//录音开始
$("body").on("click",".yuan.on.audio",function(){
    try{
...
//拼写试题的答案点击的效果
$("body").on("click",".useranswer .answer",function(){
    clearTimeout(timer);
...
//录音之后的播放
$("body").on("click",'.yuan.on.uservoice',function(){
    var questions = window.localStorage.getItem("enhomework");
    questions = JSON.parse(questions);
  //  console.log(questions.questions[homework_question_index])
    var mp3 = questions.questions[homework_question_index].useranswer.useranswer;
    mp3 = user_record_mp3_url + mp3;
    //判断方法是否存在
    window.UXinJSInterfaceSpeech.playAudio(mp3);
})
//多选题点击事件
$("body").on("click", ".item.checkbox", function() {
    clearTimeout(timer);
    //如果这个选项被选择
    // if($(this).find(".option_circle").hasClass("active")){
    //     return ;
    // }
    //如果答案已经填满
    if($(this).parents(".xuanze").find(".focus").length == 0){
        mui.toast('请选择要修改的答案进行替换',{ duration:'long', type:'div' }) 
        return;
    }
    var answer = $(this).find(".option_circle").text();
    var itemindex = $(this).index();
    var position = $(this).parents(".xuanze").find(".focus").attr("position");
    if(position){
        $(this).parents(".xuanze").find(".item.checkbox").eq(position).find(".option_circle").removeClass("active");
    }
    $(this).find(".option_circle").addClass("active");
    //将试题的答案提交到用户的表中
    $(this).parents(".xuanze").find(".focus").html(answer);
    var index = $(this).parents(".xuanze").find(".focus").index();
    $(this).parents(".xuanze").find(".focus").removeClass("focus").attr("position",itemindex);
    $(this).parents(".xuanze").find(".answer").eq(index+1).addClass("focus");
});

//多选题提交按钮
$("body").on("click",".item.submit",function(){
    clearTimeout(timer);
    //答案
    var answers = [];
    var isover = true;
    $(this).parents(".xuanze").find(".useranswer .answer").each(function(k,v){
        var answer = $(v).text();
        isover = isover && (answer == ""?true:false);
        answers.push(answer);
    })
    if(isover){
        mui.toast('答案不能为空',{ duration:'long', type:'div' }) 
        return ;
    }
    $(this).find(".option_circle").addClass("active");
    ...
    var enhomework = window.localStorage.getItem("enhomework");
    var homework_questions = JSON.parse(enhomework);
    var attrs = getAllAttr(this);
    var question = homework_questions.questions[homework_question_index];
    var questionid = question.id;
    var flag = true;
    var useranswer = {};
    useranswer.useranswer = [];
    var wordArr = question.answer;
    $.each(answers,function(k,v){
        if(v != wordArr[k]){
            flag = false;
        }
        useranswer.useranswer.push(v);
    })
    if(flag){
        useranswer.userscore = (flag == false)?0:1;
    }else{
        useranswer.userscore = 0;
    }
    useranswer.iscorrect = (useranswer.userscore == 0)?0:1;
    useranswer.spendtime = (new Date().getTime() - question_start_time)/1000;
    homework_questions.questions[homework_question_index].useranswer = useranswer;
    window.localStorage.setItem("enhomework",JSON.stringify(homework_questions));
    var user = {};
    ...
    $.post("../Mobhw/userAnswerQuestion",
        ...
        function(data){
                    ...
                    if(data.flag == "1"){
                           mui.toast('答案保存成功',{ duration:'long', type:'div' });
                             //学生进行数据的提交
                            todatika();
                    }
                    else{
                         mui.toast('答案保存失败',{ duration:'long', type:'div' });
                    }

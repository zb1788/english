...
var exams_mp3_url=window.location.protocol+"://"+document.domain+"/yylmp3/mp3_exam/";
...
//题干播放
function stem_play(examsdata,stemdata,obj){
...
function child_stem_init(examsdata,stemdata,childstemdata){
	mp.childstemindex = 0;
//	alert(mp.childinitstemindex+'=='+childstemdata.length);
...
function listen(url,stemid,examsid,obj){
	//题干的id
	$.getJSON(url,{stemid:stemid,examsid:examsid,ran:Math.random()},function(data){
		...
		mp.clear();
		$(obj).addClass('active');
		stem_play(data[0].stem,data[0].stem,obj);
	});
}

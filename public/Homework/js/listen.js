...
function headerHtml(examsdata){
	...
	temphtmlstr = '<div class="wbBtn bLine"><p id="total_score" examsid="{$examsid}" unitid="{$unitid}" total_score="{$total_score}">'+examsdata[0]['name']+'</p><p id="papernum"></p></div>';
    $(".wordbook").empty();
    $(".wordbook").html(temphtmlstr);
}
//去除HTML tag
...
function stemHtml(stemvalue){
    ...
    if(stemvalue.questypeid ==1 || stemvalue.questypeid == 3){
		...
		if(stemvalue.stem_type == '1'){
			temphtmlstr = '<h3 class="active"><a href="javascript:void(0);">'+stemvalue.content+'(每'+str+'<b>'+stemvalue.question_score+'</b>分，共<b>'+stemvalue.total_score+'</b>分）<i class="icon-volume-up" onclick="javascript:listen(\'getStemTts\','+stemvalue.id+','+stemvalue.examsid+');"></i></a></h3>';
		}
    	else{
            if(stemvalue.stem_children.length ==1){
                ...
                temphtmlstr = '<h3 class="active"><a href="javascript:void(0);">'+stemvalue.content+'<i class="icon-volume-up" onclick="javascript:volice('+stemvalue.id+');"></i></a></h3>';
            }
...
function childstemHtml(data){
	...
	if(data.content != ''){
		temphtmlstr = '<h4>'+data.content+'（每'+str+'<b>'+data.question_score+'</b>分，共<b>'+data.total_score+'</b>分）</h4>';
	}
...
function xuanzeHtml(quevalue,stem_type){
    ...
    $(itemsdata).each(function(itemskey,itemsvalue){
    	if(quevalue.itemtype == '0'){	//选项是文字
    		itemsclass = '';
    		itemshtml += '<dd><input stem_type="'+stem_type+'"  name="'+quevalue.id+'" type="radio" value="'+itemsvalue.flag+'">'+itemsvalue.flag+'.<span>'+itemsvalue.content+'</span></dd>';
    	}
    	else{	//选项是图片
    		itemsclass = 'ti_tupian';
    		itemshtml += '<dd><img src="/uploads/'+itemsvalue.content+'"><span><input stem_type="'+stem_type+'" name="'+quevalue.id+'" type="radio" value="'+itemsvalue.flag+'">'+itemsvalue.flag+'</span></dd>';
    	}
    });
    temphtmlstr += '<dl class="xuanzeti '+heshuiclass+' '+itemsclass+'">';
    if(quevalue.tcontent == ''){
    	temphtmlstr += quevalue.question_num+'、';
    }
    else{
    	temphtmlstr += '<dt>'+quevalue.question_num+'、'+quevalue.tcontent+'</dt>';
    }
...
function tiankongHtml(quevalue,stem_type){
	...
	temphtmlstr = '';
	temphtmlstr += '<p>'+quevalue.question_num+'. '+quevalue.tcontent+'</p>';
	return temphtmlstr;
}
//判断题html
function panduanHtml(quevalue,stem_type){
    ...
    temphtmlstr += '<p id="'+quevalue.id+'">'+quevalue.question_num+'. '+removeHTMLTagimg(quevalue.tcontent)+'<span style="padding-left:15px;"><input stem_type="'+stem_type+'" name="'+quevalue.id+'" type="radio" value="1"><i class="icon-checkmark"></i></span><span><input name="'+quevalue.id+'" type="radio" value="0"><i class="icon-cancel"></i></span></p>';
    return temphtmlstr;
}
//排序题html
function paixuHtml(quevalue,stem_type){
    ...
    if(quevalue.itemtype == '0'){
       paixuhtml1 = '<div class="inputs">';
    }else{
       paixuhtml1 = '<div class="imgs">';
    }

    paixuhtml2 = '';
    $(itemsdata).each(function(itemskey,itemsvalue){
	    if(quevalue.itemtype == '0'){	//选项是文字
	    	paixuhtml1 += '<p><span>'+itemsvalue.flag+'.'+itemsvalue.content+'</span></p>';
	    }
	   	else{	//选项是图片
            paixuhtml1 += '<p><span><img src="/uploads/'+itemsvalue.content+'">'+itemsvalue.flag+'.</span></p>';
	    }
	    paixuhtml2 += '<li>'+(itemskey+1)+'.<input stem_type="'+stem_type+'" type="text" name="'+quevalue.id+'"></li>';
    });
    if(quevalue.itemtype == '0'){
...
function show_paper_info(examsid) {
    ...
    $.getJSON('listenshow', {examsid: examsid,ran:Math.random()}, function(result) {
    	...
    	$(stemdata).each(function(stemkey,stemvalue){
            ...
            if(stemkey==(len-1)){
                htmlstr += '<div class="boxCon" id="submit"><p><a href="javascript:void(0);" class="btn bBlue  submit">提交答案</a></p></div>';
            }
    		htmlstr +="</div></ul></div>";
    	});
    	//
...
function updateTime ()
{
	var oRemain = parseInt($('#minutes').html()) * 60 + parseInt($('#seconds').html());
	if(oRemain <= 0)
	{
		clearInterval(timer);
		return;
	...
	oRemain %= 60;
	$('#seconds').html(format(parseInt(oRemain)));
}

...
function headerHtml(examsdata){
	...
	temphtmlstr = '<div class="titleCon"><div class="fl"><h2>'+examsdata[0]['name']+'</h2><a href="javascript:;" class="playBtn none"></a></div><div class="fr  none" id="changebutton"><a  href="javascript:;" class="aBtn cur">听力试题</a><a  href="javascript:;" class="aBtn">听力材料</a><a href="javascript:;" class="aBtn">返回</a></div><div class="clearfix"></div></div>';
    	temphtmlstr += '<div>'+examsdata[0]['header_content']+'</div>';
    	temphtmlstr +='<div class="speed"><span>速度：<input name="speed" type="radio" value="0"><label>慢速</label><input name="speed" type="radio" value="1" checked><label>正常</label><input name="speed" type="radio" value="2"><label>快速</label></span><span style="float:right;"><label id="totalscore" style="color:red">满分:'+examsdata[0]['total_score']+'分&nbsp;&nbsp;</label><span id="uptimer" style="color:red;display:none"><span id="minutes">00</span> : <span id="seconds">00</span></span></span></div>';
...
function stemHtml(stemvalue){
    ...
    if(stemvalue.questypeid ==1 || stemvalue.questypeid == 3){
		...
		if(stemvalue.stem_type == '1'){
			temphtmlstr = '<h3>'+stemvalue.content+'（每'+str+'<b>'+stemvalue.question_score+'</b>分，共<b>'+stemvalue.total_score+'</b>分）</h3>';
		}
    	else{
...
function xuanzeHtml(quevalue,stem_type){
    ...
    $(itemsdata).each(function(itemskey,itemsvalue){
    	if(quevalue.itemtype == '0'){	//选项是文字
    		...
    		itemshtml += '<dd><img src="/uploads/'+itemsvalue.content+'"><span><input stem_type="'+stem_type+'" name="'+quevalue.id+'" type="radio" value="'+itemsvalue.flag+'">'+itemsvalue.flag+'</dd>';
    	}
    });
...
function paixuHtml(quevalue,stem_type){
    ...
    $(itemsdata).each(function(itemskey,itemsvalue){
	    if(quevalue.itemtype == '0'){	//选项是文字
            ...
            paixuhtml1 += '<li><img src="/uploads/'+itemsvalue.content+'"><span>'+itemsvalue.flag+'.</span></li>';
	    }
        //alert(answer[itemskey].answer_num);
	   
    });
    $(answer).each(function(answerkey,answerval){
         paixuhtml2 += '<li>'+answerval.answer_num+'.<input stem_type="'+stem_type+'" type="text" name="'+quevalue.id+'"></li>';
    });
...
function stemttsHtml(stemvalue){
	...
	$(stemttsdata).each(function(i,value){
		temphtmlstr += '<h3 class="showtl" style="display:none;" stemid="'+stemvalue.id+'" class="examsmp3" tts_type="'+value.tts_type+'" stoptimes = "'+stemvalue.stoptimes+'" playtimes = "'+stemvalue.question_playtimes+'" st_flag="'+value.st_flag+'" mp3value="'+value.tts_mp3+'">mp3地址:'+value.tts_mp3+'</h3>';
	});
	return temphtmlstr;
...
function quettsHtml(quetiondata,stem_type,stemkey,childstemkey){
    ...
    $(quetiondata).each(function(i,value){
    	...
    	$(quettsdata).each(function(quettskey,quettsval){
            ...
            if(flag_content != ''){
                flag_content = quettsdata.flag_content+':';
            }
            else{
                flag_content = '';
            }
...
function answerHtml(quetiondata,id){
    ...
    $(quetiondata).each(function(i,value){
    	...
    	$(answerdata).each(function(answerkey,answerval){
    		...
    		temphtmlstr+='<span id="answer'+value.id+answer_num+'">'+answer_num+'. '+answer+'</span>';  
    	});
    });             
   	temphtmlstr +='</div>';
	return temphtmlstr;

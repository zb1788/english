...
var onAudioPlayStatus=function(status){
    ...
    if(status == 0){
        var len=0;
        if(mp3.getPlayliststyle()==1){
            len=mp3.getCurpageListlen();
            if(mp3.getCurpage()==(mp3.getLength()-1)&&(mp3.getCurpageListlen())==mp3.getCurindex()){
                //$(".active").find("i").removeClass("icon-uniE60C actY").addClass("icon-volume-up");
                //$(".active").removeClass("active");
                $("#audioplay").addClass("isover");
                $("#audioplay").find("font").text("再听一次");
                $("#audioplay").find("i").addClass("icon-uniE60C").removeClass("icon-playt");
        ...
        //是够开启播放完成的弹出菜单
        if(mp3.getUse()==1){
            if((len)>=mp3.getCurindex()){
                mp3time = setTimeout(function(){
                    //表示的是列表图文模式的情况
                    if(mp3.getListstyle()==1&&mp3.getPlayliststyle()==0&&mp3.getCurindex()<(mp3.getLength()-1)){
                        //$("#next").click();
                    }else if(mp3.getPlayliststyle()==1&&(mp3.getCurpageListlen())==mp3.getCurindex()){
                        $("#audioplay").attr("onplay","1");
                        $(".curplay").removeClass("curplay");
                        //$("p i.icon-uniE60C").removeClass("icon-uniE60C actY").addClass("icon-volume-up");
                        $(".active").removeClass("active");
                        if(mp3.getCurpage()<(mp3.getLength()-1)){
                            //$("#next").click();
                        }
                        //mp3.addCurpage();
                        //mp3.setCurindex(0);
                        //mp3.setParentobj($("div.text").eq(mp3.getCurpage()).find("i"));
                    }
                    mp3.playWordList(mp3.getCurindex());
                },mp3.getStoptime());
            }
        }else{

            $(".cPlay.iplay").addClass("istop").removeClass("iplay");
            // var len=$("img.curimg").length;
            // if(len>0){
            //     //$("img.curimg").attr("src","../../public/Subject/img/timg.png");

...
function showContent(style, ks_code) {
  ...
  if (style == 1) {
    ...
    $("#playbutton").attr('url',textMp3Arr[0]);
  } else if (style == 2) {
    $.each(textJson.kewen, function (k, v) {
      var contentDemo = $("#demo")
        .children("div")
    ...
    if (len > 0) {
      ...
      //alert(textJson.kewen[0].url);
      $("#playbutton").attr('url',textJson.kewen[0].url);
      $("#content").show();
...
function playKewen(obj) {
  ...
  if ($(obj).hasClass("icon-playk")) {
    ...
    $("#playimg").attr("src","/public/Yuwen/share/images/ting.png");
    if (isfirst == 1) {
      //第一次播放
...
function showBar(i) {
 ...
 try {
        UXinJSInterfaceSpeech.hideVoiceView(1);
        UXinJSInterfaceSpeech.openAudioView(1,voices,1,url,0);
      } catch (e) {
        setTip("您的APP版本过低部分功能无法使用，请升级至最新版本。");
      }

...
function subPageCss4(){   
   $subPageCss2Str="";   
    ...
    if($this->current_page > 1){   
    //调用查询页面的js方法pagelist()
    $subPageCss4Str.='<a href="javascript:;" onclick="pagelist(1,'.$this->each_disNums.')">首页</a> ';
    $subPageCss4Str.='<a href="javascript:;" onclick="pagelist('.($this->current_page-1).','.$this->each_disNums.')">上一页</a> ';
    }else {   
    $subPageCss4Str.="<a>首页</a> ";   
    $subPageCss4Str.="<a>上一页</a> ";   
    }   
   ...
   $a=$this->construct_num_Page();  
    for($i=0;$i<count($a);$i++){   
      $s=$a[$i];   
      if($s == $this->current_page ){   
          $subPageCss4Str.='<a href="javascript:;" style="background:#fff;color:black;" >'.$s.'</a>';   
       }else{   
       //调用查询页面的js方法pagelist()
       $subPageCss4Str.='<a href="javascript:;" onclick="pagelist('.$s.','.$this->each_disNums.')">'.$s.'</a>';
       }   
    }   
    if($this->current_page < $this->pageNums){
	   		//调用查询页面的js方法pagelist()
	   		$subPageCss4Str.='<a href="javascript:;" onclick="pagelist('.($this->current_page+1).','.$this->each_disNums.')">下一页</a> ';
	   		$subPageCss4Str.='<a href="javascript:;" onclick="pagelist('.$this->pageNums.','.$this->each_disNums.')">尾页</a> ';
	}else {
		$subPageCss4Str.="<a>下一页</a> ";
		$subPageCss4Str.="<a>尾页</a> ";
	} 
    $subPageCss4Str.="共".$this->nums."条&nbsp;<a>第".$this->current_page."页/共".$this->pageNums."页 </a>";
	$subPageCss4Str.='<select name="SelectPages" class="select" id="SelectPages" style="line-height: 35px; width: 40px; height: 35px; vertical-align: middle;">';
	   	
	   	for ($i=1;$i<=$this->pageNums;$i++){
	   		if($i == $this->current_page)
	   		{
	   			$selected = 'selected';
				//$stype="style='background:#fff;color:black;'";
	   		}
	   		else{
	   			$selected = '';
				//$stype='';
	   		}
	   		$subPageCss4Str.='<option value="'.$i.'" '.$selected.' >'.$i.'</option>';
	   	}
	   	
	   	$subPageCss4Str.='</select>';
		$subPageCss4Str.='<input type="hidden" id="current_page" value="'.$this->current_page.'" />';
	   	$subPageCss4Str.='<input type="hidden" id="total_page" value="'.$this->pageNums.'" />';
	   	$subPageCss4Str.='<input type="hidden" id="nums" value="'.$this->nums.'" />';
		$subPageCss4Str.='<input type="hidden" id="eachnums" value="'.$this->each_disNums.'" />';
		$subPageCss4Str.='<script>$("#SelectPages").change(function(){var eachnums=$("#eachnums").val();var curnum=$(this).val();pagelist(curnum,eachnums);})</script>';
